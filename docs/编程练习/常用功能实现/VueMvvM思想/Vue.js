...
function Observe(data) {
  ...
  for (let key in data) {
    ...
    Object.defineProperty(data, key, {
      ...
      enumerable: true,
      get() {
        //* 哪个Watcher触发get，Dep.tartget就是指向谁，这里把Watcher添加到中心管理者Dep
        //* 为什么这样写是因为我们会有多长get取值的过程，我们只想在监听取值的时候才订阅
      ...
      if (node.nodeType === 3 && reg.test(txt)) {
        ...
        function replaceTxt() {
          node.textContent = txt.replace(reg, (matched, placeholder) => {
            ...
            //* 当html里有{{key}}来插入数据时，我们就需要监听这个key
...
function Watcher(vm, exp, fn) {
  ...
  let arr = exp.split(".");
  let val = vm;
  //* 这一步是关键，可能{{obj.a.b.c}}这种链式访问，需要循环取值
  //* 取值就会进入我们的数据劫持阶段里的get，所以是通过观察者来触发数据劫持的
  arr.forEach(key => {
    // 取值
    val = val[key]; // 获取到this.a.b，默认就会调用get方法
  });
  //* 等上面取值进入get执行完相关逻辑，回来时将Dep.target置空
  Dep.target = null;
}
